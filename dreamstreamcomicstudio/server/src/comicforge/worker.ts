...
const handleJob = async (job: { id?: string; name: string; data: Record<string, unknown> }) => {
  ...
  try {
    await comicForgePipelineService.notifyJobProgress(job.id, 20, 'Preparing worker payload');
    const result = await comicForgePipelineService.processJob(
      job.id,
      job.name as TaskType,
      job.data
    );
    await comicForgePipelineService.notifyJobProgress(job.id, 90, 'Finalizing artifacts');
