...
{"note":"one short sentence on what you fixed","files":[{"path":"<path>","content":"<full corrected file content>"}]}`;
};
...
export const parseFixResponse = (text: string): StudioFixResult => {
  ...
  return { files, note: typeof parsed?.note === 'string' ? parsed.note : undefined };
};
