...
2. **Full library on demand (optional):** run Open Design's **MCP server** (`od mcp install …`) and
   point the studio at it via `STUDIO_OPENDESIGN_MCP_URL` to give agents the full 142+ design
   systems + 100+ skills as tools. It's a `stdio` server, so front it with an HTTP bridge like the
   other design MCPs (see `deploy/studio-tools/`).
