...
| 4 | [Agentic build loop](./phases/PHASE-4-agentic-loop.md) | 🟡 **core shipped** — observation parser + guards + orchestrator + FIX stage + coding-model router (39 tests); live `/api/studio/build` SSE route + client trace pending | live route needs signed-in validation |
