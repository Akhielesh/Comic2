...
const TONE = { good: '#059669', bad: '#dc2626', neutral: '#64748b' } as const;
...
// parameters, and its tool/agent surface, in the house comic style.
...
const ParamRow: React.FC<{ p: RecipeParameterView }> = ({ p }) => (
  <li className="flex items-start gap-2 py-1">
    <span className="mt-0.5 text-slate-400">{paramIcon(p.input_type)}</span>
    <span className="min-w-0 flex-1">
      <span className="flex flex-wrap items-center gap-1.5">
        <code className="text-[11px] font-bold">{p.key}</code>
        <Badge color={p.requirement === 'required' ? TONE.bad : TONE.neutral}>{p.requirement}</Badge>
        <span className="text-[10px] uppercase text-slate-400">{p.input_type}</span>
        {p.default !== undefined && <span className="text-[10px] text-slate-400">default: {String(p.default)}</span>}
      </span>
      ...
      header={
        <div className="flex items-start gap-2">
          ...
          <span className="min-w-0">
            <span className="flex flex-wrap items-center gap-1.5">
              <span className="font-display text-[15px] font-extrabold leading-tight">{data.title}</span>
            ...
            <span className="mt-0.5 block text-[12px] text-slate-600">{data.description}</span>
            {data.id && <code className="text-[10px] text-slate-400">{data.id}</code>}
          </span>
        </div>
      }
    >
      <div className="px-3 pb-2">
        {(data.tools?.length || data.agents?.length) ? (
          <div className="mb-2 flex flex-wrap items-center gap-1">
            {data.agents?.map((a) => (
              <span key={a} className="inline-flex items-center gap-0.5 rounded border border-fuchsia-200 bg-fuchsia-50 px-1.5 py-0.5 text-[10px] font-bold text-fuchsia-700">
                <Bot className="w-3 h-3" />
                {a}
              </span>
            ))}
            {data.tools?.map((t) => (
              <span key={t} className="inline-flex items-center gap-0.5 rounded border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 text-[10px] font-bold text-emerald-700">
        ...
        {params.length > 0 && (
          <div>
            <div className="mb-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-400">Parameters</div>
      ...
      {(data.instructions || (data.activities && data.activities.length > 0)) && (
        <Expandable moreLabel="Show instructions" lessLabel="Hide instructions">
          <div className="space-y-2 px-3 py-2">
            {data.instructions && (
              <div>
                <div className="mb-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-400">Instructions</div>
                <p className="whitespace-pre-wrap text-[11px] leading-relaxed text-slate-600">{data.instructions}</p>
              </div>
            )}
