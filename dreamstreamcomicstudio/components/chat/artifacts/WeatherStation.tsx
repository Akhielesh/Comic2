...
import { Surface, Chart, RadialGauge, LinearGauge, Compass, SunArc, Badge } from './kit';
import type { ChartPoint } from './kit';
import { InlineMap } from './InlineMap';

// Flagship weather card. A full station readout: animated sky hero, a gauge grid
// for every sensor (UV, humidity, wind compass, pressure, visibility, cloud, dew,
// precip), an interactive hourly temperature chart, a 7-day forecast, air-quality +
// pollen bars, a sunrise/sunset arc, and a map view. All dependency-free SVG.
...
const Tile: React.FC<{ icon: React.ReactNode; label: string; children: React.ReactNode }> = ({ icon, label, children }) => (
  <div className="flex flex-col items-center gap-1 rounded-lg border border-black/5 bg-slate-50/70 p-2 text-center">
        ...
        <div className="flex gap-1 px-3 pb-1 pt-2">
          {([['forecast', 'Forecast', BarChart3], ['map', 'Map', MapIcon]] as const).map(([id, label, Icon]) => (
            <button
              ...
              onClick={() => setTab(id)}
              aria-pressed={tab === id}
              className={`flex items-center gap-1 rounded-md border-2 px-2 py-0.5 text-[11px] font-bold transition-colors ${
                tab === id ? 'border-black bg-slate-900 text-white' : 'border-black/15 text-slate-500 hover:border-black/40'
              }`}
            >
        ...
        <>
          ...
          <div className={`relative overflow-hidden bg-gradient-to-br ${bg} px-4 py-4 text-white`}>
            ...
            <div className="relative z-10">
              <div className="flex justify-end">
                <div className="flex overflow-hidden rounded-full border border-white/50 text-[11px] font-bold">
                  {(['C', 'F'] as const).map((u) => (
                    <button key={u} onClick={() => setUnit(u)} aria-pressed={unit === u} className={`px-2 py-0.5 transition-colors ${unit === u ? 'bg-white text-slate-900' : 'text-white/90 hover:bg-white/20'}`}>
                      °{u}
                    </button>
                  ))}
                </div>
              ...
              <div className="-mt-3 flex items-end justify-between">
                <div>
                  <div className="font-display text-5xl leading-none">
                    {t(c.tempC)}°<span className="align-top text-2xl">{unit}</span>
                  </div>
                  <div className="mt-1 text-sm font-bold">{c.description}</div>
          ...
          <div className="grid grid-cols-2 gap-2 p-3 sm:grid-cols-4">
            {typeof uv === 'number' && (
              <Tile icon={<Sun className="h-3 w-3" />} label="UV index">
                ...
                <span className="text-[10px] font-bold" style={{ color: uvBand(uv).color }}>{uvBand(uv).label}</span>
              </Tile>
            )}
            ...
            {typeof c.humidity === 'number' && (
              <Tile icon={<Droplets className="h-3 w-3" />} label="Humidity">
                <span className="font-display text-2xl">{c.humidity}%</span>
                <div className="mt-1 w-full px-1"><LinearGauge value={c.humidity} max={100} color="#0ea5e9" /></div>
                {typeof c.dewPointC === 'number' && <span className="mt-1 text-[10px] text-slate-500">Dew {t(c.dewPointC)}°</span>}
              </Tile>
            )}
            ...
            {typeof c.precipProb === 'number' && (
              <Tile icon={<Umbrella className="h-3 w-3" />} label="Precip">
                <span className="font-display text-2xl">{c.precipProb}%</span>
            ...
            {typeof c.cloudCover === 'number' && (
              <Tile icon={<Cloud className="h-3 w-3" />} label="Cloud">
                <span className="font-display text-2xl">{c.cloudCover}%</span>
            ...
            {typeof c.visibilityKm === 'number' && (
              <Tile icon={<Eye className="h-3 w-3" />} label="Visibility">
                <span className="font-display text-2xl">{c.visibilityKm}</span>
            ...
            {typeof c.feelsLikeC === 'number' && (
              <Tile icon={<Thermometer className="h-3 w-3" />} label="Feels like">
                <span className="font-display text-2xl">{t(c.feelsLikeC)}°</span>
                <span className="text-[10px] text-slate-500">Actual {t(c.tempC)}°</span>
              </Tile>
            )}
          </div>
          ...
          {hourlyPoints.length > 2 && (
            <div className="px-1 pb-1">
              <div className="px-2 text-[10px] font-bold uppercase tracking-wide text-slate-400">Next 24 hours</div>
          ...
          {/* 7-day forecast */}
          {data.daily.length > 0 && (
            <div className="border-t-2 border-black/10 px-3 py-2">
                ...
                {data.daily.slice(0, 7).map((d) => {
                  ...
                  return (
                    <div key={d.date} className="flex items-center gap-2 text-xs">
                      <span className="w-9 font-bold text-slate-600">{dayName(d.date)}</span>
                      ...
                      <span className="w-7 text-right font-semibold text-slate-400">{lo}°</span>
                      ...
                      <span className="w-7 font-bold">{hi}°</span>
                    </div>
                  );
                })}
              </div>
          ...
          {(aq || pollen || today?.sunrise) && (
            <div className="grid gap-3 border-t-2 border-black/10 bg-slate-50/50 p-3 sm:grid-cols-2">
              {aq && (aq.usAqi !== undefined || aq.euAqi !== undefined) && (
                <div>
                  <div className="mb-1 flex items-center justify-between">
                    <span className="flex items-center gap-1 text-[11px] font-bold text-slate-600"><Gauge className="h-3.5 w-3.5" /> Air quality</span>
              ...
              {pollen && pollen.level && (
                <div>
                  <div className="mb-1 flex items-center gap-1 text-[11px] font-bold text-slate-600"><Leaf className="h-3.5 w-3.5 text-emerald-500" /> Pollen · {pollen.level}</div>
                  <div className="space-y-1">
                    {([['Tree', pollen.tree], ['Grass', pollen.grass], ['Weed', pollen.weed]] as const).map(([lbl, val]) => (
                      <div key={lbl} className="flex items-center gap-2">
                        <span className="w-9 text-[10px] text-slate-500">{lbl}</span>
              ...
              {today?.sunrise && (
                <div className="sm:col-span-2">
                  <div className="mb-0.5 flex items-center gap-1 text-[11px] font-bold text-slate-600"><Sun className="h-3.5 w-3.5 text-amber-500" /> Sun</div>
