...
const Section: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
  <div>
    <div className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--ds-muted)] mb-1.5">{icon} {title}</div>
    {children}
  </div>
);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={load}
          className="flex items-center gap-1 text-[11px] font-semibold text-[var(--ds-ink)] rounded-lg border border-[var(--ds-hairline)] bg-[var(--ds-surface-soft)] px-2.5 py-1 transition-colors duration-200 hover:bg-[var(--ds-hover)]"
        >
          ...
          {data.capabilities.map((c) => (
            <li key={c.id} className="flex items-start gap-2 text-sm">
              ...
              <span className="min-w-0">
                <span className="font-semibold text-[var(--ds-ink)]">{c.label}</span>
                <span className="block text-[11px] text-[var(--ds-muted)]">{c.detail}{c.envVar && c.status !== 'ok' ? ` · add ${c.envVar}` : ''}</span>
              </span>
            </li>
          ))}
        </ul>
      ...
      <Section icon={<Plug className="w-3.5 h-3.5" />} title="Live tool-API health">
        <div className="grid sm:grid-cols-2 gap-1.5">
          {data.toolHealth.map((t) => (
            <div key={t.id} className="flex items-center gap-2 rounded-xl border border-[var(--ds-hairline)] bg-[var(--ds-raised)] px-2.5 py-1.5 text-sm">
              {t.ok ? <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" /> : <XCircle className="w-4 h-4 text-red-600 shrink-0" />}
              <span className="font-semibold text-[var(--ds-ink)] truncate flex-1">{t.label}</span>
              <span className="text-[11px] text-[var(--ds-muted)] tabular-nums">{t.ok ? `${t.latencyMs}ms` : (t.error || `HTTP ${t.status}`)}</span>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section icon={<Gauge className="w-3.5 h-3.5" />} title="Dependencies & limits">
        <ul className="space-y-1.5">
          {data.dependencies.map((d) => (
            <li key={d.id} className="rounded-xl border border-[var(--ds-hairline)] bg-[var(--ds-raised)] px-2.5 py-1.5">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-sm text-[var(--ds-ink)] flex-1">{d.label}</span>
                <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-md border ${d.connected ? 'border-emerald-500/20 text-emerald-700 bg-emerald-50' : 'border-[var(--ds-hairline)] text-[var(--ds-muted)] bg-[var(--ds-well)]'}`}>
                  {d.connected ? 'connected' : 'not connected'}
                </span>
              </div>
              <div className="text-[11px] text-[var(--ds-muted)] mt-0.5">{d.freeTier}</div>
              {d.live ? (
                <div className="text-[11px] font-semibold text-[var(--ds-ink)] mt-0.5 tabular-nums">
      ...
      <Section icon={<Gauge className="w-3.5 h-3.5" />} title="Rate limits">
        <div className="flex flex-wrap gap-2">
          {data.rateLimits.map((r) => (
            <span key={r.scope} className="text-[11px] font-semibold text-[var(--ds-muted)] rounded-full border border-[var(--ds-hairline)] bg-[var(--ds-surface-soft)] px-2.5 py-0.5 tabular-nums">
        ...
        <Section icon={<AlertTriangle className="w-3.5 h-3.5" />} title={`Recent capability gaps (${data.recentNotices.length})`}>
          <ul className="space-y-1">
            {data.recentNotices.slice(0, 15).map((n, i) => (
              <li key={i} className="text-[11px] text-[var(--ds-muted)] flex items-start gap-1.5">
                <span className={`mt-0.5 w-1.5 h-1.5 rounded-full shrink-0 ${n.level === 'error' ? 'bg-red-500' : 'bg-amber-500'}`} />
                <span><span className="font-semibold text-[var(--ds-ink)]">{n.tool || 'system'}</span>: {n.message}{n.at ? ` · ${new Date(n.at).toLocaleTimeString()}` : ''}</span>
              </li>
            ))}
          </ul>
        </Section>
