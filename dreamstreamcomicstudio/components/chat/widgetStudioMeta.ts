 ...
 *  `agent_activity` is generated by the chat agent's tool loop at runtime (a live trace
 *  of the tools it ran), so — like `swarm_trace` — it has no single producing tool. */
export const MODEL_AUTHORED = new Set(['swarm_trace', 'agent_activity', 'recipe_card', 'recipe_run', 'research_report']);
