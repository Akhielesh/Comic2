    ...
    return (
        <header className="h-20 bg-white border-b-4 border-black flex items-center px-6 justify-between relative z-50 shadow-sm sticky top-0">
            ...
            <div className="flex items-center gap-4">
                {isEditorMode && onBack && (
                     <Button size="sm" variant="outline" onClick={onBack} icon={<ArrowLeft size={16} />}>
                        Back
                     </Button>
                )}
                ...
                {isEditorMode && projectTitle && (
                    <>
                        <div className="h-8 w-[2px] bg-slate-200 hidden md:block"></div>
                        <div className="font-display text-lg hidden md:block truncate max-w-xs">
                            {projectTitle}
                        </div>
                    </>
                )}
