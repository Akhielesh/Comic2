...
import { extractWorldDetails } from '../../services/geminiService';
...
export const ReferenceBuilder: React.FC<ReferenceBuilderProps> = ({ 
    scenes, currentStyle, projectId, initialCharacters, initialItems, initialLocations, onDataUpdate, onConfirm 
}) => {
  ...
  const handleGenerateAll = async () => {
      const list = activeTab === 'characters' ? characters : activeTab === 'items' ? items : locations;
      ...
      await runWithLimit(entitiesToGenerate, 3, async (entity) => {
        await generateEntityImage(activeTab, entity.id);
      });
